...
const ManageRooms = () => {
  ...
  return (
    <div className="p-4 md:p-10 max-w-[1200px] mx-auto flex flex-col gap-6 text-white pb-24">
      <div className="flex justify-between items-end">
        ...
        <button onClick={() => setShowAddModal(true)} className="size-10 rounded-full bg-primary flex items-center justify-center shadow-lg shadow-primary/20"><span className="material-symbols-outlined">add</span></button>
      </div>

      {/* Mobile Card View (Visible on Small Screens) */}
      <div className="grid grid-cols-1 gap-3 md:hidden">
        {rooms.map(room => (
            <div key={room.id} className="bg-[#16212b] p-4 rounded-[2rem] border border-[#223649] flex items-center justify-between">
      ...
      <div className="hidden md:block overflow-hidden rounded-3xl border border-[#223649] bg-[#16212b]">
        <table className="w-full text-left">
            <thead className="bg-[#1c2a38] text-slate-400 text-xs">
                <tr>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
            <div className="bg-[#16212b] p-8 rounded-t-[3rem] sm:rounded-3xl w-full max-w-md border-t sm:border border-[#223649] animate-in slide-in-from-bottom duration-300">
                <h3 className="text-xl font-black mb-6">เพิ่มห้องพักใหม่</h3>
                <form onSubmit={handleAddSubmit} className="flex flex-col gap-4">
                    <input required className="bg-[#1c2a38] border-none rounded-2xl h-14 px-6 text-white" value={newRoom.id} onChange={e => setNewRoom({...newRoom, id: e.target.value})} placeholder="หมายเลขห้อง" />
                    <select className="bg-[#1c2a38] border-none rounded-2xl h-14 px-6 text-white" value={newRoom.type} onChange={e => setNewRoom({...newRoom, type: e.target.value})}>
                        <option>Standard</option>
                        <option>Standard Twin</option>
                    </select>
                    <input required type="number" className="bg-[#1c2a38] border-none rounded-2xl h-14 px-6 text-white" value={newRoom.price} onChange={e => setNewRoom({...newRoom, price: Number(e.target.value)})} placeholder="ราคาต่อคืน" />
                    <div className="flex gap-3 mt-4">
                        ...
                        <button type="submit" className="flex-1 py-4 rounded-2xl bg-primary text-white font-black shadow-lg shadow-primary/20">บันทึก</button>
                    </div>
                </form>
            </div>
        </div>
